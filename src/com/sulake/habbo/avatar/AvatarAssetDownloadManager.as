package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_1216:Dictionary;
      ...
      private const const_2241:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         this.var_979 = new Timer(this.const_2241,1);
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_445)
         {
            ...
            this.var_445.addEventListener(AssetLoaderEvent.const_41,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_445)
         {
            ...
            this.var_445.removeEventListener(AssetLoaderEvent.const_41,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         if(this.var_977 <= 0)
         {
            HabboWebTools.logEventLog("Figuremap download error " + this.var_1215);
            Logger.log("AVATAR ASSET ERROR: Could not download figuremap.xml.");
         }
         else
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_977);
            if(this.var_1215.indexOf("?") > 0)
            {
               _loc2_ = this.var_1215 + "&retry=" + this.var_977;
            }
            else
            {
               _loc2_ = this.var_1215 + "?retry=" + this.var_977;
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2580,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1216[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1216[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1862)
         {
            this.var_1214.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_503[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_503[_loc3_] = _loc6_;
            }
            this.var_978[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         for(_loc3_ in this.var_978)
         {
            ...
            if(_loc7_)
            {
               ...
               delete this.var_503[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_978[_loc3_];
         }
         while(_loc6_ < this.var_790.length)
         {
            _loc5_ = this.var_790[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_790.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_979.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_789.length > 0 && this.var_790.length < this.const_1775)
         {
            _loc1_ = this.var_789[0];
            _loc1_.startDownloading();
            this.var_790.push(this.var_789.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_789.indexOf(param1) == -1 && this.var_790.indexOf(param1) == -1)
         {
