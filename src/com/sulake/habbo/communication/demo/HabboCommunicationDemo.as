package com.sulake.habbo.communication.demo
{
   ...
   public class HabboCommunicationDemo extends Component
   {
      ...
      private var var_523:IKeyExchange;
      
      private var var_968:String;
      
      private var _view:HabboLoginDemoView;
      
      private var var_2531:PseudoRandom;
      ...
      private var var_228:HabboHotelView;
      
      private var var_1483:String = "";
      
      private const const_1970:String = "fuselogin";
      ...
      public function setSSOTicket(param1:String) : void
      {
         ...
         {
            this.var_630 = param1;
            this.var_57.initConnection(HabboConnectionType.const_239);
         }
      }
      ...
      {
         if(this.var_57 && this.var_138 && this.var_16 && this._windowManager)
         {
      ...
      private function componentsAreRunning() : void
      {
         ...
         var _loc1_:Boolean = this.var_138.getBoolean("client.hotel_view.show_on_startup",true);
         this.var_228 = new HabboHotelView(this._windowManager,assets,Component(context).events,_loc1_);
         var _loc2_:String = this.var_138.getKey("client.hotel_view.image");
         var _loc3_:String = this.var_138.getKey("image.library.url");
         if(_loc2_ != null && _loc3_ != null)
         {
            this.var_228.loadHotelViewImage(_loc3_ + _loc2_);
         }
         var _loc4_:* = this.var_138.getKey("use.sso.ticket") == "1";
         this.var_630 = this.var_138.getKey("sso.ticket",null);
         this.var_1818 = this.var_138.getKey("flash.client.url","");
         this.var_2532 = this.var_138.getKey("external.variables.txt","");
         if(this.var_228 == null)
         {
            this.var_228 = new HabboHotelView(this._windowManager,assets,Component(context).events,_loc1_);
         }
         ...
         else if(this._windowManager != null)
         {
            ...
            if(_loc5_)
            {
               ...
               this._view.addEventListener(HabboLoginDemoView.const_464,this.onInitConnection);
            }
            else
            {
               Core.crash("Login without an SSO ticket is not supported",Core.const_1691);
            }
         }
      }
      ...
      private function onInitConnection(param1:Event = null) : void
      {
         ...
         {
            this.var_57.mode = HabboConnectionType.const_96;
            if(this.var_138.keyExists("local.environment"))
            {
               this.var_57.mode = HabboConnectionType.const_1326;
            }
         }
         this.var_57.initConnection(HabboConnectionType.const_239);
      }
      ...
      private function onConnectionEstablished(param1:Event = null) : void
      {
         ...
         if(_loc2_ != null)
         {
            this.dispatchLoginStepEvent(HabboCommunicationEvent.const_770);
      ...
      private function onInitCrypto(param1:IMessageEvent) : void
      {
         ...
         this.var_2531 = new PseudoRandom(parseInt(_loc4_.substring(_loc4_.length - 4),16),65536);
         switch(this.var_57.mode)
         {
            case HabboConnectionType.const_96:
               _loc6_ = "";
               _loc6_ = this.var_138.getKey("url.prefix",_loc6_);
               _loc6_ = _loc6_.replace("http://","");
               _loc6_ = _loc6_.replace("https://","");
               _loc7_ = this.var_138.getKey("hotelview.banner.url","http:/\nsitename$/gamedata/banner");
               _loc7_ = _loc7_.replace("$sitename$",_loc6_);
               this.var_1483 = _loc4_;
               this.var_228.loadBannerImage(_loc7_ + "?token=" + this.var_1483,this.onHotelViewBannerLoaded);
               break;
            case HabboConnectionType.const_1326:
               this.sendConnectionParameters(_loc2_);
               break;
            case HabboConnectionType.const_1800:
               this.var_968 = this.generateRandomHexString(30);
               this.var_523 = new DiffieHellman(new BigInteger(this.var_138.getKey("connection.development.prime"),16),new BigInteger(this.var_138.getKey("connection.development.generator"),16));
               this.var_523.init(this.var_968);
               _loc8_ = this.var_523.getPublicKey(10);
      ...
      private function onSecretKeyEvent(param1:IMessageEvent) : void
      {
         ...
         this.var_523.generateSharedKey(_loc4_,10);
         var _loc5_:String = this.var_523.getSharedKey(16).toUpperCase();
         var _loc6_:RC4_R27 = new RC4_R27(null,null);
         var _loc7_:IEncryption = new RC4_R27(_loc6_,this.var_2531);
      ...
      private function sendConnectionParameters(param1:IConnection) : void
      {
         ...
         connection.send(new VersionCheckMessageComposer(401,this.var_1818,this.var_2532));
         var machineId:String = "";
         try
         {
            so = SharedObject.getLocal(this.const_1970,"/");
      ...
      private function onConnectionDisconnected(param1:Event) : void
      {
         ...
         if(param1.type == Event.CLOSE && !this.var_2533)
         {
            _loc2_ = this.var_138.getKey("logout.disconnect.url");
      ...
      private function onDisconnectReason(param1:DisconnectReasonEvent) : void
      {
         ...
         this.var_2533 = true;
         var _loc2_:String = this.var_138.getKey("logout.url");
         if(_loc2_.length > 0)
         {
      ...
      private function setOriginProperty(param1:String) : String
      {
         ...
         {
            return param1.replace("%origin%",this.var_138.getKey("flash.client.origin","popup"));
         }
         return param1;
      }
      ...
      private function onHotelViewBannerLoaded(param1:BitmapData) : void
      {
         ...
         var _loc4_:String = xor(_loc3_,this.var_1483);
         ...
         this.var_523 = new DiffieHellman(_loc10_,_loc11_);
         var _loc13_:int = 10;
         var _loc14_:* = null;
         while(_loc13_ > 0)
         {
            _loc14_ = this.generateRandomHexString(30);
            this.var_523.init(_loc14_);
            _loc15_ = this.var_523.getPublicKey(10);
