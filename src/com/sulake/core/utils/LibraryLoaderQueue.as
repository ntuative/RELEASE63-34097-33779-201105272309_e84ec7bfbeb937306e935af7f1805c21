package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      {
         return this.var_453.length + this.var_290.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_290.length < const_1775 && this.var_453.length > 0)
            {
               _loc1_ = this.var_453.shift();
               this.var_290.push(_loc1_);
               _loc1_.resume();
            }
         }
      }
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            ...
            index = this.var_290.indexOf(loader);
            if(index > -1)
            {
               this.var_290.splice(index,1);
            }
         }
         catch(e:Error)
         {
      ...
      public function findLibraryLoaderByURL(param1:String, param2:Boolean = true) : LibraryLoader
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_290)
            {
               if(param2)
               {
                  if(_loc3_.url.indexOf(param1) == 0)
